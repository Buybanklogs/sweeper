...
/**
 * Auto-fix old localhost cache
 */
const savedApiBase = localStorage.getItem(
  "EVM_WALLETCONNECT_API_BASE_URL"
);
...
function apiBaseUrl() {
  let value = elements.apiBaseUrl.value
    .trim()
    .replace(/\/$/, "");
...
function updateActions() {
  ...
  const synced =
    Boolean(state.syncResult?.chains?.length);

  const hasDetectedAssets =
    detectedAssets().length > 0;

  const hasPreparedTransfers =
    Boolean(state.sweepPlan?.transfers?.length);

  elements.connectWallet.disabled =
    !hasConsent || connected;

  elements.syncWallet.disabled =
    !connected;

  elements.disconnectWallet.disabled =
    !connected;

  elements.prepareSweep.disabled =
    !synced || !hasDetectedAssets;

  elements.approveSweep.disabled =
    !hasPreparedTransfers ||
    !state.provider;
}
...
function parseSessionChains(
  ...
  const chainIds = namespaceAccounts
    ...
    .filter((chainId) =>
      supported.has(chainId)
    );

  if (
    provider.chainId &&
    supported.has(Number(provider.chainId))
  ) {
    chainIds.push(
...
async function initProvider(config) {
  ...
  const provider =
    await EthereumProvider.init({
      ...
      methods:
        config.walletConnect.methods,

      optionalMethods:
        config.walletConnect.methods,

      events:
        config.walletConnect.events,
      ...
      metadata: {
        name:
          "EVM Infrastructure Access",

        description:
          "WalletConnect session for balance review and wallet-approved transfers.",
...
async function connectWallet() {
  if (
    ...
    setStatus(
      "Consent is required before connection."
    );
...
elements.consentCheckbox.addEventListener(
  "change",
  () => {
    updateActions();
...
elements.connectWallet.addEventListener(
  "click",
